...
class LearningWorkflowIT {
    ...
    void getsPracticeItemDetail() throws Exception {
        mockMvc.perform(get("/api/v1/learning/practice-items/practice-thank-you"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.itemId").value("practice-thank-you"))
                .andExpect(jsonPath("$.data.lessonId").value("lesson-greetings-1"))
                .andExpect(jsonPath("$.data.expectedGloss").value("THANK_YOU"))
